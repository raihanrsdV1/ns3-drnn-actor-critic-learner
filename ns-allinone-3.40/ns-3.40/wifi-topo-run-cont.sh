#!/usr/bin/env bash
# =================================================================
# wifi-topo-run-cont.sh — Continuous-Action (SAC) WiFi experiment
#
# Experimental companion to wifi-topo-run.sh.
# Agent outputs a direct float cwnd target (bytes) via SAC instead
# of choosing 1-of-7 AIMD multipliers as in the discrete version.
#
# Runs:
#   1. Build  wifi-sim-cont  (continuous-action ns-3 CC class)
#   2. Cubic  baseline  (60 s simulation)
#   3. Reno   baseline  (60 s simulation)
#   4. SAC    training  (N episodes, each 60 s sim)
#   5. SAC    eval      (deterministic policy, frozen weights)
#   6. Generate comparison + training-curve plots
#
# Usage (from  ns-3.40/  directory):
#   ./wifi-topo-run-cont.sh                  # 50 training episodes, resume model
#   ./wifi-topo-run-cont.sh 30               # 30 episodes
#   ./wifi-topo-run-cont.sh 50 --fresh       # 50 episodes, discard saved model
#   ./wifi-topo-run-cont.sh --fresh          # 50 episodes, fresh start
#   ./wifi-topo-run-cont.sh 0                # baselines only, no SAC
# =================================================================
set -euo pipefail
...
MODEL="$OUT_BASE/wifi_cont_model.pth"
TRAINLOG="$CSV_DIR/w_cont_train_log.csv"
PORT=5557
# Reward / cwnd parameters for the WiFi parking-lot topology
BOTTLENECK_MBPS=4.0
MIN_RTT_MS=72.0
CWND_MIN_KB=4.0     # ⅓ × BDP/flow — safety floor
CWND_MAX_KB=24.0    # 2 × BDP/flow — range centered on optimal ~12 KB

echo "============================================================"
echo "  WiFi Parking-Lot TCP Experiment  (Continuous SAC variant)"
echo "  Topology: 3-STA WiFi → AP → R1 ─[8M]─ R2 ─[4M]─ Server"
echo "  Agent   : LSTM-SAC, cwnd ∈ [${CWND_MIN_KB} KB, ${CWND_MAX_KB} KB]"
echo "  Training episodes : $N_EPISODES"
echo "  Simulation time   : 60 s per episode"
echo "============================================================"

# ── Step 1: Build ────────────────────────────────────────────────
echo -e "\n[1/6] Building wifi-sim-cont (continuous-action CC class)..."
./ns3 build "$SIM"
echo "      Build OK."
...
if [ "$N_EPISODES" -gt 0 ]; then
    ...
    for ep in $(seq 1 "$N_EPISODES"); do
        ...
        # Extract SAC alpha and avg reward from agent log
        alpha=$(grep "SAC α" "/tmp/wifi_agent_ep${ep}.log" \
...
for f in w_cont_*.csv; do [ -f "$f" ] && mv "$f" "$CSV_DIR/"; done
echo "      Eval done → w_cont_*.csv / w_drnn_*.csv"
...
echo -e "\n============================================================"
echo "  Done!"
echo "  • wifi_full_comparison.png   — Cubic vs Reno vs SAC-Continuous"
echo "  • wifi_training_curves.png   — SAC training progress"
echo "  • w_cubic_*.csv              — Cubic baseline"
echo "  • w_reno_*.csv               — Reno baseline"
echo "  • w_cont_*.csv               — SAC eval results"
echo "  • w_cont_train_log.csv       — Per-episode training metrics"
echo "============================================================"
