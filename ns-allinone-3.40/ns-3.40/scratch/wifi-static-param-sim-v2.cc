/*
 * wifi-static-param-sim.cc -- Parameterizable WiFi 802.11 Static Simulation
 *
 * Topology (scalable WiFi infrastructure):
 *
 *   STA0 --+                                                 +-- Dst0
 *   STA1 --+-- (WiFi 802.11a) -- AP -- [10Mbps/10ms/64p] -- R1 -- [100Mbps/1ms] --+-- Dst1
 *   ...  --+                                                 +-- ...
 *
...
main (int argc, char *argv[])
{
  ...
  // WiFi CSMA/CA simulation is O(N_sta) per packet event — cap STAs at 20 to
  // keep runtime reasonable for large nNodes sweeps (60-100 nodes).
  // Extra nNodes beyond the cap are modelled as additional wired destinations,
  // preserving the node-count effect on routing/addressing without ballooning
  // the WiFi contention domain.
  static const uint32_t MAX_STA = 20;
  uint32_t rawSTA = (nNodes - 2) / 2;
  uint32_t nSTA   = std::min (rawSTA, MAX_STA);
  uint32_t nDst   = nNodes - 2 - nSTA;   // remainder all go to wired side
  ...
  if (nFlows > nSTA)
    NS_LOG_UNCOND ("NOTE: nFlows=" << nFlows << " > nSTA=" << nSTA
                   << "; flows assigned round-robin across " << nSTA << " STAs.");
  ...
  // ======================= WiFi 802.11a (OFDM, 5 GHz) =======================
  // Using 802.11a instead of 802.11n: removes HT overhead (A-MPDU aggregation,
  // Block ACK, HT PHY headers) which causes 5-10x simulation slowdown.
  // 802.11a OFDM at 54 Mbps is sufficient — the 10 Mbps wired bottleneck is
  // the limiting factor anyway, so HT throughput gains don't matter.
  ...
  // Human-readable
  std::cout << "\n=== WiFi 802.11 Static Simulation Complete ===\n"
    << "Transport:     " << transport << "\n"
  ...
  // SUMMARY,transport,nNodes,nFlows,pps,coverageMult,
