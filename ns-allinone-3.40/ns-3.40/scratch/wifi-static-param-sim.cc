...
main (int argc, char *argv[])
{
  ...
  // WiFi topology: all "left-half" nodes are WiFi STAs, rest are wired destinations.
  // More STAs = more WiFi contention = more drops/delay/energy. No artificial cap.
  ...
  // STA devices — ActiveProbing=true so STAs probe the AP directly
  // instead of waiting for beacons (lets us reduce beacon rate).
  mac.SetType ("ns3::StaWifiMac",
               "Ssid", SsidValue (ssid),
               "ActiveProbing", BooleanValue (true));
  NetDeviceContainer staDevices = wifi.Install (wifiPhy, mac, staNodes);

  // AP device — reduce beacon rate from default 102.4ms to every 10s.
  // With ActiveProbing STAs don't need frequent beacons to associate.
  // This saves ~100 beacon events/sec × N_sta receiver events.
