 ...
 *   Updated every 100 ms (dt=0.1 s). Agent picks 1 of 7 AIMD multipliers.
 */
...
// ═══════════════════════════════════════════════════════════════════
// CUSTOM CC: TcpWifiDrnn
// LSTM agent controls cwnd via 7 AIMD-multiplier actions.
// Action  →  AIMD multiplier on base rate (segSize²/cwnd per ACK)
//   0 = FAST_DEC   (-4×)    drain queues quickly
//   1 = SLOW_DEC   (-1×)    gentle pullback
//   2 = MAINTAIN   ( 0×)    hold cwnd
//   3 = AIMD       (+1×)    standard TCP CA
//   4 = MOD_INC    (+2×)    moderate growth
//   5 = FAST_INC   (+4×)    fast growth
//   6 = VERY_FAST  (+8×)    startup ramp-up
// ═══════════════════════════════════════════════════════════════════
...
public:
  ...
  // Set by OpenGym ExecuteAction each step
  static uint32_t g_action;
};

uint32_t TcpWifiDrnn::g_action = 3; // default: AIMD

// BDP_LINK2 = 4 Mbps × (1+10+25+25+10+1) ms ≈ 36,000 bytes per flow
// Allow 4× BDP headroom before hard cap: 144 KB
...
{
  // On packet loss, halve cwnd (standard AIMD loss response).
  // The agent learns to avoid losses — this fires only as a safety net.
  return std::max<uint32_t> (2 * tcb->m_segmentSize,
                              tcb->m_cWnd.Get () / 2);
}
...
TcpWifiDrnn::IncreaseWindow (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
  ...
  // Base AIMD increment: +1 MSS per RTT (same as standard TCP CA)
  int32_t base = std::max<int32_t> (1, (int32_t)((seg * seg) / cwnd));
  ...
  int32_t newCwnd = (int32_t)cwnd + base * mult;
  tcb->m_cWnd = std::min<uint32_t> (
    DRNN_MAX_CWND,
    std::max<uint32_t> (2 * seg, (uint32_t)std::max<int32_t> (0, newCwnd)));
}
...
// Observation: 9 floats — for each flow: [cwnd_bytes, rtt_ms, bytesInFlight]
static const uint32_t OBS_DIM = NUM_FLOWS * 3;
...
GetObsSpace ()
{
  // 9 continuous values: [cwnd0, rtt0, inflight0, cwnd1, rtt1, inflight1, ...]
  // Use the constructor: OpenGymBoxSpace(low, high, shape, dtype)
...
GetActSpace ()
{
  Ptr<OpenGymDiscreteSpace> space = CreateObject<OpenGymDiscreteSpace> (7);
  return space;
}
...
ExecuteAction (Ptr<OpenGymDataContainer> action)
{
  ...
  if (disc)
    TcpWifiDrnn::g_action = disc->GetValue ();
  return true;
}
